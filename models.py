...
class HyperND(nn.Module):
    ...
    def __init__(self,args):
        ...
        self.classifier = MLP(in_channels=self.num_features,
    ...
    def diffusion(self, data):
        ...
        D = torch_scatter.scatter_add(ones, V, dim=0)+self.self_loop

        ones = torch.ones(E.shape[0], device=device, dtype=torch.float32)
        DE = torch_scatter.scatter_add(ones, E, dim=0)+self.self_loop
        ...
        def E2V(Z):
            ...
            #Xv.masked_fill_(Xv==float('inf'),0.)
            return Xv
...
#         Note that add dropout to attention is default in the original paper
    ...
    def forward(self, data):
        ...
        x = data.x
        edge_index = data.edge_index
        if hasattr(data, 'icd_weight'):
            icd_weight=data.icd_weight
        else:
            icd_weight=None

        for i, conv in enumerate(self.convs[:-1]):
            x = F.elu(conv(x,edge_index,hypericd_weight=icd_weight))
            x = F.dropout(x, p=self.dropout, training=self.training)

#         x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.convs[-1](x, edge_index,hypericd_weight=icd_weight)
