...
#         x = torch.matmul(torch.diag(data.D_v_beta), x)
        ...
        out = self.propagate(hyperedge_index, x=x, norm=data.D_e_beta_inv,edge_weight=icd_weight,
...
#         out = torch.matmul(torch.diag(data.D_e_alpha), out)
    ...
    def message(self, x_j, norm_i, edge_weight: Optional[Tensor] = None):
...
class HypergraphConv(MessagePassing):
    ...
    def forward(self, x: Tensor, hyperedge_index: Tensor,
        ...
        if not self.symdegnorm:
            ...
            out = self.propagate(hyperedge_index, x=out, norm=D, alpha=alpha,size=(num_nodes, num_edges))
